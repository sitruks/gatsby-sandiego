    ...
    return (
      <div className='cs-modal-clues'>
        <div className='cs-modal-clues-inner'>
          <div className='cs-modal-clues-building'>
            {clues.map((clue, index) => (
              <span
                key={index}
                onClick={() =>
                  clue.Clue === '' ? this.viewArrest() : this.viewClue(clue)
                }
                data-key={index}
                role='img'
                className={
                  clue.Place.key === this.state.selectedItem
                    ? 'cs-modal-clues-emoji cs-selected'
                    : 'cs-modal-clues-emoji'
                }
                aria-label='emoji'
              >
                {this.emoji(clue.Place.key)}
              </span>
            ))}
          </div>
          {clues.length === 0 ? (
            <div className='cs-modal-clues-places'>no clue here...</div>
          ) : (
            ''
          )}
          <div className='cs-modal-clues-places'>
            {clues.map((clue, index) => (
              <div
                key={index}
                data-key={index}
                onMouseOver={() => this.hov(clue.Place.key)}
                onMouseOut={() => this.hov(clue.Place.key)}
                onClick={() =>
                  clue.Clue === '' ? this.viewArrest() : this.viewClue(clue)
                }
              >
          ...
          <br />
          <button
            className='cs-modal-clues-close'
            type='button'
            onClick={() => hideModalClue()}
          >
