...
class ModalMap extends Component {
  ...
  componentDidMount() {
    ...
    this.props.destinations.forEach((item, index) => {
      let marker = {
        city: item,
        x: this.myDiv.getElementsByClassName(
          item.toLowerCase().replace(' ', '')
        )[0].offsetLeft,
        y:
          339 -
          this.myDiv.getElementsByClassName(
            item.toLowerCase().replace(' ', '')
          )[0].offsetTop
      }
      markers.push(marker)
  ...
  render() {
    ...
    if (this.props.isPlane) {
      xArrivee = this.myDiv.getElementsByClassName(
        this.props.place.toLowerCase().replace(' ', '')
      )[0].offsetLeft
      yArrivee =
        339 -
        this.myDiv.getElementsByClassName(
          ...
          <div ref={this.mymap}>
            ...
            {destinations.map((item, index) => (
              <div
                id={
                  hoverDestination &&
                  hoverDestination.toLowerCase().replace(' ', '') ===
                    item.toLowerCase().replace(' ', '')
                    ? 'hovDestination'
                    : ''
                }
...
class Plane extends Component {
  render() {
    ...
    const planeClasses = isPlane
      ? `cs-plane cs-active ${cityStart.toLowerCase().replace(' ', '')}`
      : 'cs-plane'
    return (
