...
import ModalClues from './ModalClues'
import ModalCrime from './Crime'
import soundHover from './Sounds/hover.mp3'
...
class Buttons extends Component {
  ...
  async goTo(destination) {
    ...
    const depart = this.state.markers.filter(
      (item) => item.city === 'depart'
    )[0]
    const arrivee = this.state.markers.filter(
      (item) => item.city === destination
    )[0]
    console.log(depart)
    console.log(arrivee)
    const distanceDepartArrivee = Math.trunc(
      Math.sqrt(
        Math.pow(arrivee.x - depart.x, 2) + Math.pow(arrivee.y - depart.y, 2)
      )
    )
    console.log(
  ...
  render() {
    ...
    const destinations = isWrongDestination
      ? [Cases[userCase][lang].steps[userStep].city.name]
      : Cases[userCase][lang].steps[userStep].destinations
    const cityStart = isWrongDestination
      ? tmpDestination
      : Cases[userCase][lang].steps[userStep].city.name
    const clues = isWrongDestination
      ? []
      : Cases[userCase][lang].steps[userStep].clues
    return (
      <div>
        ...
        <div className='cs-btns'>
          <div
            onMouseEnter={() => this.soundHover.play()}
            className={
              this.state.isShowModalDestinations
                ? 'cs-btn cs-btn-hidden-mobile cs-btn-hide '
                : 'cs-btn cs-btn-see cs-btn-hidden-mobile'
            }
            onClick={this.showModalDestinations}
          >
          ...
          <div
            ...
            className={
              this.state.isShowModalDepart
                ? 'cs-btn cs-btn-depart cs-btn-depart-hide'
                : 'cs-btn cs-btn-depart  cs-btn-depart-show'
            }
            onClick={this.showModalDepart}
          >
            <div className='cs-btn-inner'></div>
          </div>
          <div
            ...
            className={
              this.state.isShowModalClue
                ? 'cs-btn cs-btn-search cs-btn-hide'
                : 'cs-btn cs-btn-search cs-btn-see'
            }
            onClick={this.showModalClue}
          >
            <div className='cs-btn-inner'></div>
          </div>
          <div
            ...
            className={
              this.state.isShowModalCrime
                ? 'cs-btn cs-btn-crime cs-btn-hide'
                : 'cs-btn cs-btn-crime cs-btn-see'
            }
            onClick={this.showModalCrime}
          >
            <div className='cs-btn-inner'></div>
          </div>
        </div>
        <div>
          <audio preload='auto' ref={this.setSoundHover} src={soundHover}>
            {' '}
          </audio>
        </div>
        {this.state.isShowModalDestinations ? (
          <ModalDestinations
            lang={lang}
            depart={this.state.isDepart}
            destinations={destinations}
            hide={this.hideModalDestinations}
          ></ModalDestinations>
        ) : (
          ''
        )}
        {this.state.isShowModalDepart ? (
          <ModalDestinations
            goTo={this.goTo}
            hoverTo={this.hoverTo}
            lang={lang}
            depart={this.state.isDepart}
            destinations={destinations}
            hide={this.hideModalDepart}
          ></ModalDestinations>
        ) : (
          ''
        )}
        {this.state.isShowModalMap && !isWrongDestination ? (
          <ModalMap
            isPlane={this.state.isPlane}
            place={place}
            hoverDestination={this.state.hoverDestination}
            changeMarkersDestinations={this.changeMarkersDestinations}
            lang={lang}
            cityStart={cityStart}
            destinations={destinations}
          ></ModalMap>
        ...
        {this.state.isShowModalClue ? (
          <ModalClues
            ...
            clues={clues}
            viewClue={viewClue}
            viewArrest={viewArrest}
            hideModalClue={this.hideModalClue}
          ></ModalClues>
        ) : (
          ''
        )}
