...
import Cases from '../GameData/CarmenGameData/Cases'
...
import soundClock from './Sounds/clock.mp3'
import './Styles/_Interface.css'
...
class Interface extends Component {
  ...
  onChangeStep(step) {
    ...
    if (step === 20) {
      this.changePlace(
        Cases[this.state.userCase][this.state.lang].steps[this.state.userStep]
          .city.name
      )
    }
  }
  ...
  async changeDate(hours) {
    ...
    let speed = 500
    if (
      this.state.nextdate.getHours() > 22 ||
      this.state.nextdate.getHours() < this.state.mydate.getHours()
    ) {
  ...
  checkDestination = (destination) => {
    if (this.state.isWrongDestination) {
      ...
      Cases[this.state.userCase][this.state.lang].steps[this.state.userStep + 1]
  ...
  restart = () => {
    ...
    this.changePlace(
      Cases[this.state.userCase][this.state.lang].steps[0].city.name
    )
  }
    ...
    return (
      <div>
        <Menu
          ...
          hydratestateFromLocalStorage={this.hydratestateFromLocalStorage}
          Dossiers={Dossiers}
          Config={Config}
          lang={lang}
        ...
        <Container>
            ...
            <Col md='5'>
              ...
              <section className='cs-zone cs-zone-2'>
                <div className='cs-zone-inner'>
                  ...
                  {step === 20 ? <Picture place={place}></Picture> : ''}
                </div>
              </section>
            </Col>
            <Col md='7'>
          ...
          <audio ref={this.setMySoundClock} src={soundClock}></audio>
        </Container>
      </div>
    )
  }
