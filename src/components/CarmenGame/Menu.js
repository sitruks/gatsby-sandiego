...
import ModalDossiers from './ModalDossiers'
  ...
  return (
    <div className='container cs-no-padding'>
        ...
        <Navbar.Collapse id='basic-navbar-nav'>
          <Nav className='mr-auto'>
            <NavDropdown title={menu.entry1.name} id='basic-nav-dropdown'>
              ...
              <NavDropdown.Item
                onClick={() =>
                  setTimeout(() => hydratestateFromLocalStorage(), 2000)
                }
              >
                {menu.entry1.item4}{' '}
              </NavDropdown.Item>
              <NavDropdown.Divider />
              <NavDropdown.Item onClick={() => setModalCredits(false)}>
                {menu.entry1.item1}
              </NavDropdown.Item>
            </NavDropdown>
            <NavDropdown title={menu.entry2.name} id='basic-nav-dropdown'>
              {Dossiers.map((dossier) => (
                <NavDropdown.Item
                  key={dossier[lang].id}
                  onClick={() => {
                    setModalDossier(false)
                    setLastOpenDossier(dossier[lang].id)
                  }}
                >
                  {dossier[lang].name}
                </NavDropdown.Item>
            ...
            <NavDropdown title='Options' id='basic-nav-dropdown'>
              ...
              <NavDropdown.Item onClick={() => changeLang('fr')}>
                {menu.entry3.item2}{' '}
              </NavDropdown.Item>
      ...
      {!modalDossier ? (
        <ModalDossiers
          lang={lang}
          hideModalDossier={setModalDossier}
          lastOpenDossier={lastOpenDossier}
        ></ModalDossiers>
      ) : (
        ''
      )}
      {!modalCredits ? (
        <ModalCredits
          lang={lang}
          hideModalCredits={setModalCredits}
        ></ModalCredits>
      ) : (
        ''
      )}
